...
function Beneficiaries() {
  ...
  const handleSubmit = async (payload) => {
    try {
      ...
      if (selectedBeneficiary) {
        response = await updateBeneficiary(selectedBeneficiary.id, payload);
      } else {
  ...
  const handleDelete = async (beneficiary) => {
    ...
    try {
      const response = await deleteBeneficiary(beneficiary.id);

      if (!response.success) {
        showToast(
          response.error || "Impossible de supprimer le bénéficiaire.",
          "error",
        );
  ...
  return (
    <div className="page beneficiaries-page">
      <PageHeader
        ...
        actions={
          <Button onClick={openCreateModal}>+ Nouveau bénéficiaire</Button>
        }
      />
      ...
      <div className="beneficiary-stats">
        ...
        <div className="beneficiary-stat-card">
          ...
          <strong>{filteredBeneficiaries.length}</strong>
        </div>
      </div>
