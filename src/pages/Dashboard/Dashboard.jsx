...
function Dashboard() {
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        ...
        const [banksResponse, beneficiariesResponse, transfersResponse] =
          await Promise.all([getBanks(), getBeneficiaries(), getTransfers()]);
  ...
  const activeBanks = banks.filter((bank) => Boolean(bank.is_active));
  ...
  const currencyTotals = useMemo(() => {
    ...
    transfers.forEach((transfer) => {
      ...
      totals[currency] = (totals[currency] || 0) + Number(transfer.amount || 0);
    });
        ...
        <>
          <div className="dashboard-stats">
            <div className="dashboard-stat">
              ...
              <small>ordres enregistrés</small>
            </div>
            ...
            <div className="dashboard-stat">
              ...
              <small>banques disponibles</small>
            </div>
            ...
            <div className="dashboard-stat">
              ...
              <small>toutes devises confondues</small>
            </div>
          </div>
              ...
              {recentTransfers.length === 0 ? (
                ...
                <div className="recent-transfer-list">
                  {recentTransfers.map((transfer) => (
                    <div className="recent-transfer" key={transfer.id}>
                      <div>
                        <strong>{transfer.reference}</strong>

                        <span>
                          {transfer.beneficiary_name || "Bénéficiaire"}
                        </span>
                      </div>
          ...
          >
            <div className="dashboard-bank-list">
              {banks.map((bank) => (
                <div className="dashboard-bank" key={bank.id}>
                  ...
                  <div>
                    ...
                    <span>{bank.account || "Compte non renseigné"}</span>
                  </div>

                  <Badge variant={bank.is_active ? "success" : "neutral"}>
