...
function Settings() {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (response.success) {
        ...
        showToast(
          response.error || "Impossible de charger les paramètres.",
          "error",
        );
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    if (!settings?.id) {
      showToast("Identifiant des paramètres introuvable.", "error");
    ...
    try {
      ...
      const payload = {
        ...
        next_transfer_number: Number(settings.next_transfer_number) || 1,
      };

      const response = await updateSettings(settings.id, payload);

      if (!response.success) {
        showToast(
          response.error || "Impossible d’enregistrer les paramètres.",
          "error",
        );
      ...
      <div className="page settings-page">
        ...
        <Card>
          <div className="empty-state">
            ...
            <p>Aucun enregistrement de configuration n'est disponible.</p>
          </div>
        </Card>
      </div>
    );
        ...
        >
          <div className="form-grid-2">
            ...
            <Input
              ...
              value={settings.attijari_account || ""}
              onChange={handleChange}
            />
          </div>
        </Card>
        ...
        >
          <div className="settings-number-box">
            <Input
              ...
              value={settings.next_transfer_number || 1}
              onChange={handleChange}
            />
            ...
            <div className="settings-reference-preview">
              ...
              <strong>
                {String(settings.next_transfer_number || 1).padStart(6, "0")}
              </strong>
            </div>
          </div>
        </Card>
